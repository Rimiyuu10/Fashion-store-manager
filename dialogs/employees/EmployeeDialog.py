...
from dialogs.employees import EmployeeDetailDialog, EmployeeSummary
from models import QuyenHan, NhanVien
from states import UserState


class EmployeeDialog(QDialog):
    def __init__(self, parent=None):
        super(EmployeeDialog, self).__init__(parent)
    ...
    def show_new_dialog(self):
        if UserState.get_user().quyen_han != QuyenHan["QUAN_LY"]:
            ...
            return
        dialog = EmployeeDetailDialog(None, self)
        dialog.exec()
        self.update_employees()
    ...
    def update_employees(self):
        ...
        if self.sort_box.currentIndex() == 1:
            ...
            order_by = "ten_nv DESC"
        if self.search_box.text() != "":
            if self.search_box.text().isdigit():
                self.employees = NhanVien.fetch_all(where={"id": self.search_box.text()}, offset=(self.current_page - 1) * self.perpage, limit=self.perpage, order_by=order_by)
                self.total = NhanVien.count(where={"id": self.search_box.text()})
            else:
                self.employees = NhanVien.fetch_all(like={"ten_nv": self.search_box.text()}, offset=(self.current_page - 1) * self.perpage, limit=self.perpage, order_by=order_by)
                self.total = NhanVien.count(like={"ten_nv": self.search_box.text()})
        else:
            self.total = NhanVien.count()
            self.employees = NhanVien.fetch_all(offset=(self.current_page - 1) * self.perpage, limit=self.perpage, order_by=order_by)
        ...
        for i in range(3):
            for j in range(2):
                employee = EmployeeSummary(self.employees[i * 2 + j] if (i * 2 + j < len(self.employees)) else None, self)
                self.top_mid.addWidget(employee, i, j)
