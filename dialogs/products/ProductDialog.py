...
from dialogs.products import ProductDetailDialog, ProductSummary
from models import QuyenHan, SanPham
...
class ProductDialog(QDialog):
    ...
    def show_new_dialog(self):
        if UserState.get_user().quyen_han != QuyenHan["QUAN_LY"]:
            ...
            return
        dialog = ProductDetailDialog(None, self)
        dialog.exec()
        self.update_products()
    ...
    def update_products(self):
        ...
        if self.search_box.text() != "":
            if self.search_box.text().isdigit():
                self.products = SanPham.fetch_all(where={"id": self.search_box.text()}, offset=(self.current_page - 1) * self.perpage, limit=self.perpage, order_by=order_by)
                self.total = SanPham.count(where={"id": self.search_box.text()})
            else:
                self.products = SanPham.fetch_all(like={"ten_sp": self.search_box.text()}, offset=(self.current_page - 1) * self.perpage, limit=self.perpage, order_by=order_by)
                self.total = SanPham.count(like={"ten_sp": self.search_box.text()})
        else:
            self.total = SanPham.count()
            self.products = SanPham.fetch_all(offset=(self.current_page - 1) * self.perpage, limit=self.perpage, order_by=order_by)
        ...
        for i in range(2):
            for j in range(5):
                product = ProductSummary(self.products[i * 5 + j] if i * 5 + j < len(self.products) else None, self)
                self.top_mid.addWidget(product, i, j)
